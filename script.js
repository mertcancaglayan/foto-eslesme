...
const fotoNumaralari = generateRandomNumbers().sort(() => Math.random() - 0.5); // sıraları her zaman aynı olacağı için rastgele sort etmesi için yazdım
...
function generateRandomNumbers() {
	...
	while (randomArray.length < 8) {
		...
		randomArray.push(randomNum, randomNum);
	}
...
function kartTiklama(olay) {
	...
	if (acikKartImg.src === secilenKartImg.src) {
		...
		// görev 1, html e embeded code ekledim sonrasında display none verdim, oyun bittiğinde tekrardan display edip sayfanın ortasına aldım
		if (puan === 4) {
			const kutlamaGorseli = document.querySelector("#kutlamaGorseli");
			kutlamaGorseli.style.position = "absolute";
			kutlamaGorseli.style.top = "0";
			kutlamaGorseli.style.left = "0";
			kutlamaGorseli.style.top = "50%";
			kutlamaGorseli.style.left = "50%";
			kutlamaGorseli.style.transform = "translate(-50%, -50%)";
			kutlamaGorseli.style.display = "flex";

			setTimeout(() => {
				kutlamaGorseli.remove();
			}, 5000);
		}
